...
class Init {
                        ...
                        [col span="9" span__sm="12"]
                            <?php
                                if(!empty($gallery) and is_array($gallery)){
                                    ?>
                                    [ux_slider bullets="false" class="gallery-doan"]
                                        <?php
                                    ...
                                    [adminz_slider_custom 
                                        ...
                                        slide_width__sm="33%" 
                                        slide_width__md="25%" 
                                        as_nav_for=".gallery-doan" 
                                        slide_item_padding="4px" 
                                        slide_align="left"
                                        bullets="false"
                                        ]
                                        <?php
    ...
    function custom_element_duannoibat(){
        add_action( 'fbc_flatsome_custom_blog_col_inner_after', function($repeater,$the_query){
            if(!isset($the_query->query['post_type']) or $the_query->query['post_type'] !== 'do_an') return;
        ...
        add_action('wp_footer', function(){
            ?>
            <style type="text/css">
                .blog-posts-custom-element-wrapper.do_an .post-item.grid-col .col-inner{
                    position: relative;
                }
                .blog-posts-custom-element-wrapper.do_an .post-item.grid-col .col-inner .doanabsolue{
                    position: absolute;
                    width: 100%;
                    height: 100%;
                    display: flex;
                ...
                .blog-posts-custom-element-wrapper.do_an .post-item.grid-col .col-inner:hover .doanabsolue{
                    opacity: 1;
                }
                .blog-posts-custom-element-wrapper.do_an .post-item.grid-col .col-inner .doanabsolue .wrap{
                    display: flex;
                    gap: 10px;
                }
                .blog-posts-custom-element-wrapper.do_an .post-item.grid-col .col-inner .doanabsolue .wrap:before{
    ...
    function set_flatsome_custom_blog_title_before($args){
        $terms = wp_get_post_terms( get_the_ID(), '_bo_mon' );
