...
class ADMINZ_Helper_Taxonomy_Sync {
    ...
    function init(){
        ...
        add_action('init',function(){
            ...
            if(!$this->get_post_type_object) return;
    ...
    function update_term_by_post_type($post_id){
    	remove_action( 'edited_'.$this->taxname, [$this, 'update_post_type_by_term'],10,2);
    	remove_action( 'pre_delete_term', [$this, 'delete_post_type_by_term'],10,2);
        ...
        if($post->post_status == 'publish'){
            if($term){
                ...
                // auto renew slug
                remove_action( 'save_post_'.$this->post_type, [$this,'update_term_by_post_type'], 10, 3 );
            ...
            // update meta keys
            update_term_meta($termid,$this->term_metakey_thumbnail,get_post_thumbnail_id());
    ...
    function update_post_type_by_term($termid, $taxonomy){
        ...
        remove_action( 'save_post_'.$this->post_type, [$this,'update_term_by_post_type'], 10, 1 );
        wp_update_post(
            [
                'ID'=> $featured_id,
    ...
    function delete_post_type_by_term( $termid, $taxonomy){
        if($taxonomy !== $this->taxname) return ;
            ...
            if ($column_name === $this->term_meta_key) {
                ...
                if($post_id){
                    ...
                    <strong style="color: darkred;">
                        Please set a <?php echo $this->get_post_type_object()->labels->name; ?> to sync
                    </strong>
                    <?php
                }
            }
